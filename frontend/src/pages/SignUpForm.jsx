...
import { topics, languages } from '../utils/multiselect-options';
...
const SignUpForm = () => {
  ...
  const [details, setDetails] = useState({organization:'', industry:"engineering", status:"student"}) //Stores occupation,organisation, degree or college
  const [discoverable, setIsDiscoverable] = useState(true);
...
const topicsDefault = selectedTopics.map((el) => {
  ...
  const handleNextStep = () => {
    if (step === 1 && (!email.trim() || !name.trim() || !password.trim() || !confirmPassword.trim())) {
    ...
    if(step===3 && (!gender.trim() || !(age.toString().trim()) || !(details.status) || !(details.status.trim()))){
      toast.error('Please fill in all the required fields.');
      return;
    }

    if(step===4 && (!desc.trim() || !(details.industry) || !(details.industry.trim()) || !(details.organization) || !(details.organization.trim()))){
      toast.error('Please fill in all the required fields.');
      return;
    }
  ...
  const handleSubmit = (e) => {
    ...
    navigator.geolocation.getCurrentPosition((position)=>{
        ...
        const data ={
          coordinates,description: desc.trim(),
          name,
          password, confirmPassword,
          languages: selectedLanguages, topics:selectedTopics, email, phone, organization: details.organization, status: details.status, industry: details.industry,age, gender, discoverable
        }
    
        console.log(data)
        signup(data);
    }, (error)=>{
        toast.error("Sign-up failed as we couldn't track your location😔");
        console.log(error)
    });
...
{step === 3 && (
      ...
      >
        ...
        {step===4 && (<>
          <div className="mb-8">
              ...
              <select
        id="status"
        className="px-2 py-1 border rounded w-full" value={details.industry} onChange={(e)=>setDetails(prev => {return {...prev, industry:e.target.value}})} defaultValue={"engineering"}
      >
        ...
        <option value="not-applicable">Not Applicable</option>
            ...
            <div className="mb-8">
              ...
              <input type="text" 
        id="status"
        className="px-2 py-1 border rounded w-full" value={details.organization} onChange={(e)=>setDetails(prev => {return {...prev, organization:e.target.value}})}
      />
            </div>
    ...
    <label className="flex items-center cursor-pointer">
      <input
        ...
        onChange={()=>setIsDiscoverable(prev => !prev)}
      />
      <span className="ml-2 text-sm">Be discoverable by others</span>
    </label>
    </div>
    <div className="flex justify-end">
